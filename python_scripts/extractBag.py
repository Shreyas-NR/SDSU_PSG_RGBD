...
def read_bag_file(filename, output_dir, save_npy=True, save_png=True, stream_frames=False):
    if not stream_frames:
        out_dir, rgb_npy_dir, depth_npy_dir, fused_npy_dir, rgb_png_dir, depth_png_dir, fused_png_dir = create_output_directories(output_dir)
    ...
    try:
        while True:
            ...
            color_frame_u = frames.get_color_frame()
            ...
            color_image_u = np.asanyarray(color_frame_u.get_data())
            depth_frame = depth_frame.copy()
            depth_image_colorized = depth_image_colorized.copy()
            color_image = color_image.copy()
            color_image_u = color_image_u.copy()
            color_image = cv2.cvtColor(color_image, cv2.COLOR_BGR2RGB)
            color_image_u_cvt = cv2.cvtColor(color_image_u, cv2.COLOR_BGR2RGB)
            ...
            if not stream_frames:
                ...
                colorFrames.append(color_image_u)
        ...
        if save_npy:
            save_frames_as_npy(fusedFrames, fused_npy_dir)
        ...
        if save_png:
            ...
            save_frames_as_png(depthColorizedFrames, depth_png_dir)
...
def main():
    ...
    if choice == '1':
    # Stream frames
        read_bag_file(filename, outdir, save_npy=False, save_png=False, stream_frames=True)
    elif choice == '2':
        # Save frames
        read_bag_file(filename, outdir, save_npy=True, save_png=True, stream_frames=False)
